...
public class EnterMarks extends JFrame implements ActionListener
{
    ...
    EnterMarks()
    {
        ...
        ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("Icon/images.jpeg"));
        Image i2 = i1.getImage().getScaledInstance(150,150,Image.SCALE_DEFAULT);
        ImageIcon i3 = new ImageIcon(i2);
        JLabel image = new JLabel(i3);
        image.setBounds(500, 40, 600, 300);
        add(image);
        ...
        lblcourse.setBounds(370,65,200,20);
        lblcourse.setFont(new Font("serif",Font.BOLD,17));
        add(lblcourse);

        JLabel lblcou = new JLabel();
        lblcou.setBounds(450,65,150,20);
        lblcou.setFont(new Font("Monospaced",Font.BOLD,17));
        add(lblcou);
        ...
        try {
            Conn c = new Conn();
            PreparedStatement ps = c.c.prepareStatement("select * from student where Marks= ?");
             ps.setString(1," ");
             ResultSet rs= ps.executeQuery();
            //ResultSet rs = c.s.executeQuery("select * from student where Marks IS NULL");
            while(rs.next()) {
        ...
        try
        {
            ...
            while(rs.next())
            {
                lblcou.setText(rs.getString("Course"));
            }
        }
        catch(Exception e)
        {
...
//        add(cbsemester);
        ...
        l1 = new JLabel("1st Semester");
        l1.setBounds(200, 110, 150, 20);
        add(l1);
        
//        JLabel cb1 = new JLabel("Marks Inserted");
//        cb1.setBounds(400, 100, 150, 40);
//        add(cb1);
        
        cb = new JCheckBox("Marks Inserted");
        cb.setBounds(400, 100, 150, 40);
        cb.setSelected(true);
        add(cb);
    ...
    public void actionPerformed(ActionEvent e)
    {
        if (e.getSource() == submit) {
            ...
            try {
                if(cb.isSelected())
                {
                    //String qu="insert into student (Marks) values  '" + marks + "' where  Roll_No='"+crollno.getSelectedItem()+"'";
                    String qu="UPDATE student SET Marks = 'Marks' WHERE Roll_No = '"+crollno.getSelectedItem()+"'";
...
//                String query1 = "insert into subjects values('"+crollno.getSelectedItem()+"', '"+cbsemester.getSelectedItem()+"', '"+tfsub1.getText()+"', '"+tfsub2.getText()+"', '"+tfsub3.getText()+"', '"+tfsub4.getText()+"', '"+tfsub5.getText()+"')";
                String query1 = "insert into subjects values('"+crollno.getSelectedItem()+"', '"+l1.getText()+"', '"+tfsub1.getText()+"', '"+tfsub2.getText()+"', '"+tfsub3.getText()+"', '"+tfsub4.getText()+"', '"+tfsub5.getText()+"')";
                String query2 = "insert into marks values('"+crollno.getSelectedItem()+"', '"+l1.getText()+"', '"+tfmarks1.getText()+"', '"+tfmarks2.getText()+"', '"+tfmarks3.getText()+"', '"+tfmarks4.getText()+"', '"+tfmarks5.getText()+"')";
